...
The second MapReduce sort the trigrams and indices by document to concatenate adjacent trigrams in MapReduce 3:
...
The third MapReduce concatenates the adjacent trigrams within a blog post. Mapper 3 concatenates adjacent trigram into a meme. Before emitting the memes, a function trimPrepositions trims off prepositions in the beginning and end of the meme, and a function stopwordRate checks how many of the words in the meme are stop words. If stop words take more than 65\% of the words in the meme, the meme is ignored. In the reducer 3, too short phrases ($\leqq 5$) and too long phrases ($\geqq 200$) are ignored as they are often timestamps, advertisement, or boiler plate text. The final MapReduce is illustrated as follows:
...
The final output is key, value pairs of a meme and the list of the document ids that the meme appears.
...
We will use Support Vector Machine for meme ranking. Utilizing the LIBSVM package, we will learn C-SVM and regard each probability value as the ranking score. For better performance, we will scale feature values and use grid search to find suitable parameters.
