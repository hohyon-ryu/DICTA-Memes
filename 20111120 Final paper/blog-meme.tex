...
Our processed blog data contains more than 4.6 million memes, and they are highly sparse. Using default values for the parameters and very generous distance measures for $t_1,t_2$, we anticipated more than one million canopies. Our attempts to run the code caused out of memory errors while running on Hadoop MapReduce. Because its time complexity is near $O(N^2)$ with many canopies, we could not use the original canopy clustering implementation. After numerous attempts, we decided to develop Indexed MapReduce Canopy.
