...
\subsubsection{Defect 2: Final output}
The final output of LDA implementation is only the probability of a word given a topic, $p(w|k)$. We need to generates gamma file including document $\times$ topic matrix, which is variational posterior Dirhcilet distrubition. This result is necessary to infer the topic distribution for unseen documents using learned topic model.
