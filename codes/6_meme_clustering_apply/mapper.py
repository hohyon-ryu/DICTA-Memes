    ...
    if len(candis)==0:
      if len(candisOnes)==0:
        ...
        continue
      	...
      	CurCandid=candisOnes.pop()
        sys.stderr.write("'%s' placed into CandisOne %d, %s\n" % (meme, CurCandid, str(clusterCentsList[CurCandid])) )
        print str(CurCandid)+"\t"+meme
        candisOnes.add(CurCandid)
    else:
      sys.stderr.write("'%s' placed into Candis %d, %s\n" % (meme, candis[0], str(clusterCentsList[candis[0]])) )
